...
{
    public class ExternalUserCacheStrategy : IExternalUserCacheStrategy
    {
        private readonly IRedisCache _redisCache;
        private readonly IExternalUserRepository _externalUserRepository;
        private readonly IUnitOfWork _unitOfWork;
        public ExternalUserCacheStrategy(IRedisCache redisCache, IExternalUserRepository externalUserRepository, IUnitOfWork unitOfWork)
        {
        ...
        public async Task UnBindExternalUser(int clientId, string userId, string loginProvider)
        {
            ...
            await _redisCache.KeyDeleteAsync(GetExternalUsersRedisKey(clientId, userId));
            await _redisCache.KeyDeleteAsync(GetExternalUserRedisKey(clientId, externalUser.ProviderKey));
        }
        ...
        /// 根据第三方OpenId查询绑定信息
        /// </summary>
        public async Task<ExternalUserEntity> GetExternalUser(int clientId, string providerKey)
        {
            var key = GetExternalUserRedisKey(clientId, providerKey);
            var externalUserEntity = await _redisCache.GetAsync<ExternalUserEntity>(key);
        ...
        /// 根据用户Id查询绑定信息
        /// </summary>
        public async Task<List<ExternalUserEntity>> GetExternalUsers(int clientId, string userId)
        {
            var key = GetExternalUsersRedisKey(clientId, userId);
            var externalUsers = await _redisCache.GetAsync<List<ExternalUserEntity>>(key);
